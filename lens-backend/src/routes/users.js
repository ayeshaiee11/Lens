...
router.patch('/', async (req, res, next) => {
  try {
    ...
    const current = await db.prepare('SELECT * FROM users WHERE id = ?').get(req.userId);
    ...
    if (nextEmail !== current.email) {
      const taken = await db.prepare('SELECT id FROM users WHERE email = ? AND id != ?').get(nextEmail.toLowerCase(), req.userId);
      if (taken) return res.status(409).json({ error: 'That email is already in use.' });
    }

    await db.prepare('UPDATE users SET name = ?, email = ? WHERE id = ?').run(nextName, nextEmail.toLowerCase(), req.userId);
